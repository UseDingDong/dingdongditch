"""Fresh Google Maps constrained-control pointer and search benchmark."""
...
def expected_url() -> list[Expectation]:
    return [Expectation(
...
def wait_visible(backend: PlaywrightBackend, label: str, locator: Locator) -> None:
    result = run(
        ...
        Operation(
            ...
            expectations=expected_url(), page_precondition=PRECONDITION,
        ),
    )
...
def main() -> int:
    ...
    try:
        ...
        for label, locator in (
            ("search", SEARCH), ("zoom_in", ZOOM_IN), ("zoom_out", ZOOM_OUT)
        ):
            wait_visible(backend, f"wait_{label}_visible", locator)
        ...
        for index in range(1, 4):
            if previous is not None:
                time.sleep(2)
            result = run(backend, f"readiness_{index}", observation(index))
            if result.plan_verdict.value != "VERIFIED":
        ...
        for index, (label, request, locator) in enumerate(declarations, 1):
            ...
            if evidence["position_verification"].get("verified") is not True:
                raise RuntimeError(f"pointer {label} was not position-verified")
            pointers.append({
                "label": label,
        ...
        if "mogadishu" not in text.lower() or "somalia" not in text.lower():
            raise RuntimeError("final visible evidence lacked Mogadishu, Somalia")
